...
public class DownloadBreakPoint {
    ...
    public void downloadBreakPoint(String url, String fileName, String dirName, String tickets,
        ...
        Log.d(UploadFile.TAG, "初始化_startsPoint=" + startsPoint);
        Log.d(UploadFile.TAG, "最终_url=" + url);
        ...
        Interceptor interceptor = new Interceptor() {
            ...
            public Response intercept(Chain chain) throws IOException {
                ...
                Log.d(UploadFile.TAG, String.format("接收响应头: [%s] %n返回json:【%s】%n%s",
        ...
        Log.d(UploadFile.TAG, "发起请求");
        // 发起请求
        call = client.build().newCall(request);
        call.enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                Log.d(UploadFile.TAG, "call=" + call.request().header("Cookie"));
                Log.d(UploadFile.TAG, "e=" + e.toString());
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                Log.d(UploadFile.TAG, "服务器响应_responseCode=" + responseCode);
                Log.d(UploadFile.TAG, "服务器响应_length=" + length);
                Log.d(UploadFile.TAG, "服务器响应_startsPoint=" + startsPoint);
                Log.d(UploadFile.TAG, "服务器响应1");
                if (responseCode < 200 || responseCode >= 300) {
                    listener.fail(uploadType, -1, "下载链接中断",  callBack);
                    return;
                }
                Log.d(UploadFile.TAG, "服务器响应2");
                long sp = startsPoint;
                // 如果当前的文件大小大于文件长度，则下载的安装包有误
                if (length > NORMAL_LENGTH && length < startsPoint) {
                    Log.d(UploadFile.TAG, "服务器响应3");
                ...
                try {
                    ...
                    if (start == 0 && max == 0 && randomAccessFile.length() == 0) {
                        listener.fail(uploadType, -1, "下载链接中断",  callBack);
                        return;
                    }
                    // 下载完成
    ...
    public void download(String url, String fileName, String dirName, boolean useStream,
        ...
        if (useStream) {
            try {
                ...
                inputStream.close();
                Log.d(UploadFile.TAG, "流下载结束");
            } catch (IOException e) {
                Log.d(UploadFile.TAG, "流下载失败");
            }
        } else {
        ...
        public BufferedSource source() {
            return Okio.buffer(new ForwardingSource(originalResponse.body().source()) {
                ...
                public long read(Buffer sink, long byteCount) throws IOException {
                    ...
                    Log.d(UploadFile.TAG, "文件oldPoint=" + oldPoint);
                    Log.d(UploadFile.TAG, "文件bytesReaded=" + bytesReaded);
